...
This repo uses pnpm workspaces, so you should **always run `pnpm install` from the top-level project directory.** running `pnpm install` in the top-level project root will install dependencies for every package in the repo.
...
When making a pull request, be sure to add a changeset when something has changed. Non-packages (`examples/*`) do not need changesets.
...
_Note: Only [core maintainers (L3+)](https://github.com/Ernxst/astro-css-bundle/blob/main/GOVERNANCE.md#level-3-l3---core-maintainer) can release new versions of the package._

The repo is set up with automatic releases, using the changeset GitHub action & bot.

To release a new version of the package, find the `Version Packages` PR, read it over, and merge it.
...
### Releasing `@ernxst/astro-cssbundle@next` (aka "prerelease mode")
...
While in prerelease mode, follow the normal release process to release `@ernxst/astro-cssbundle@next` instead of `@ernxst/astro-cssbundle@latest`. To release `@ernxst/astro-cssbundle@latest` instead, see [Releasing `@ernxst/astro-cssbundle@latest` while in prerelease mode](#user-content-releasing-ernxstastro-cssbundlelatest-while-in-prerelease-mode).
...
Exiting prerelease mode should happen once an experimental release is ready to go from `npm install @ernxst/astro-cssbundle@next` to `npm install @ernxst/astro-cssbundle`. Only a core contributor run these steps. These steps should be run before
...
### Releasing `@ernxst/astro-cssbundle@latest` while in prerelease mode

When in prerelease mode, the automatic PR release process will no longer release `@ernxst/astro-cssbundle@latest`, and will instead release `@ernxst/astro-cssbundle@next`. That means that releasing to `latest` becomes a manual process. To release latest manually while in prerelease mode:
...
10. Go to <https://github.com/Ernxst/astro-cssbundle/releases/new> and create a new release. Copy the new changelog entry from <https://github.com/Ernxst/astro-cssbundle/blob/latest/packages/core/CHANGELOG.md>.
